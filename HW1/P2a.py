...
if __name__ == '__main__':
  ...
  data = None
  sample_size, image_size = 6144, sys.argv[1]
  ...
  if rank == 0:
    plt.imsave('P2a_parallel image_size:' + str(image_size) + '.png', np.mat(p_superimposed), cmap='bone')
    
    s_start = time.time()
    s_superimposed = reduce(lambda image, k: image + transformer.transform(data[k-1, :], -k*np.pi/sample_size), np.arange(numrows))
    s_stop = time.time()
    plt.imsave('P2a_serial image_size:' + str(image_size) + '.png', np.mat(s_superimposed), cmap='bone')

    rel_error = np.linalg.norm(p_superimposed - s_superimposed) / np.linalg.norm(s_superimposed)
    print "Serial Time for image size %d: %f secs" % (image_size, (s_stop - s_start))
    print "Parallel Time for image size %d: %f secs" % (image_size, (p_stop - p_start))   
